from .tables import Student, Course, Week, Lecture, Question, Assignment
    ...
    for course in Course.objects():
        ...
        for lecture in data:
            if not Lecture.objects(week=week.id, name=lecture["name"]):
                week.add_lecture(lecture_name = lecture["name"], url = lecture["video_link"], transcript = lecture["transcript"])
...
def generate_assignments():
    ...
    for course in Course.objects():
        ...
        for assignment in assignments:
            if assignment["name"] not in {_assignment.name for _assignment in week.assignments}:
                week.add_assignment(name=assignment["name"], graded=assignment["graded"], due_date=assignment["due_date"])
    ...
    for course in Course.objects():
        ...
        for question in data:
            for assignment in week.assignments:
                if not Question.objects(assignment=assignment.id, question=question["question"]):
                    assignment.add_question(question=question["question"], answers=question["answers"], correct_answer=question["correct_answer"], qtype=question["qtype"])
