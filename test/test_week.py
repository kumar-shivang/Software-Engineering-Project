...
def test_get_week(client, mocker):
    ...
    mocker.patch(
        ...
        return_value=MagicMock(first=MagicMock(return_value=MagicMock(to_json=MagicMock(return_value=mock_week))))
    )
...
def test_get_assignments(client, mocker):
    ...
    mock_week = MagicMock(assignments=[MagicMock(to_json=MagicMock(return_value=assignment)) for assignment in mock_assignments])
    mocker.patch(
        "database.tables.Week.objects",
