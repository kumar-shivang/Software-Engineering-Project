          ...
          <div className="flex flex-col h-full">
            <div className="flex-1 p-4 overflow-y-auto">
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`my-2 p-2 rounded-lg ${msg.sender === 'You' ? 'bg-blue-100' : 'bg-gray-100'}`}
                >
            ...
            <div className="p-4 border-t border-gray-200">
              <form onSubmit={handleFormSubmit} className="flex">
                ...
                />
                <button type="submit" disabled={isLoadingData} className="bg-blue-500 text-white px-4 py-2 rounded-r-lg hover:bg-blue-600">
                  Send
                </button>
              </form>
            </div>
