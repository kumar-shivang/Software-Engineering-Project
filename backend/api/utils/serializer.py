...
def serialize_doc(doc):
    ...
    for key, value in doc_dict.items():
        if isinstance(value, ObjectId):
            ...
            doc_dict[key] = serialize_doc(value)  # Recursive call for nested dictionaries
        elif isinstance(value, list):
            doc_dict[key] = [serialize_doc(v) if isinstance(v, dict) else str(v) if isinstance(v, ObjectId) else v for v in value]  # Handle lists
    return doc_dict
...
def serialize_assignment(assignment):
    return {
       "id": str(assignment.id),
        ...
        "questions": [serialize_question(question) for question in assignment.questions]
    }
...
def serialize_week(week):
    return {
        ...
        "assignments": [serialize_assignment(assignment) for assignment in week.assignments]
    }
