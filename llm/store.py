...
def chat_history_to_dict(history: BaseChatMessageHistory) -> Dict[str, Any]:
    return {'messages': [{'type': msg.type, 'data': msg.content} for msg in history.messages]}
...
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    ...
    if session is None:
        history = InMemoryChatMessageHistory()
        collection.insert_one({'session_id': session_id, 'history': chat_history_to_dict(history)})
    else:
...
def save_session_history(session_id: str, history: BaseChatMessageHistory) -> None:
    collection.update_one(
        ...
        {'$set': {'history': chat_history_to_dict(history)}}
    )